...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Renderizador simples e seguro de Markdown para respostas do bot
  function renderSafeMarkdown(input) {
    ...
    text = text.replace(/```([\s\S]*?)```/g, (m, code) => {
      ...
      codeBlocks.push(`<pre class="mk-pre"><code>${escaped}</code></pre>`);
      return token;
    });
    ...
    // Títulos (### Subtítulo)
    text = text.replace(/^###\s+(.+)$/gm, '<h3 class="mk-h3">$1</h3>');
    text = text.replace(/^##\s+(.+)$/gm, '<h3 class="mk-h3">$1</h3>');

    // Negrito e itálico (ordem importa)
    text = text.replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>');
    text = text.replace(/\*(.+?)\*/g, '<em>$1</em>');

    // Listas não ordenadas (blocos consecutivos começando com - ou •)
    text = text.replace(/(^|\n)(?:- |• )(.*)(?=\n|$)/g, (match, p1, item) => `${p1}<li>${item}</li>`);
    // Envolver grupos de <li> em <ul>
    text = text.replace(/(?:<li>[\s\S]*?<\/li>)(?:(?:\n)?(?:<li>[\s\S]*?<\/li>))*/g, (block) => `<ul class="mk-ul">${block}</ul>`);

    // Quebras de linha em parágrafos simples
    // Duas quebras viram separador de parágrafo, simples viram <br>
    text = text.replace(/\n\n+/g, '</p><p>');
    text = `<p>${text.replace(/\n/g, '<br>')}</p>`;

    // Restaurar blocos de código
    codeBlocks.forEach((html, idx) => {
      text = text.replace(`__CODEBLOCK_${idx}__`, html);
    });
  ...
  function addMsg(text, user = true, container = null) {
    ...
    const bubbleClass = user 
      ? 'max-w-[85%] rounded-lg bg-emerald-700 text-white px-3 py-2 text-sm user-bubble' 
      : 'max-w-[85%] rounded-lg bg-gray-100 text-gray-900 px-3 py-2 text-sm bot-bubble';

...
import Chat3D from './ar/chat3d.js';
...
// === SISTEMA DE CHAT 3D (modular) ===
let chat3d = null;

function create3DChatSystem() {
  if (chat3d) return;
  chat3d = new Chat3D({ scene, renderer, raycaster, tempMatrix });
  chat3d.create();
}
...
function addChatMessage(text, type = 'bot') {
  if (!chat3d) return;
  chat3d.addMessage(text, type);
}

function handleChatInteraction(controller) {
  if (!chat3d) return false;
  return chat3d.handleInteraction(controller);
}
...
function init() {
  ...
  const arWrap = document.getElementById('arButtonContainer');
  arWrap.appendChild(arButton);

  // Aplicar classe vermelha ao botão/contêiner quando o estado for "AR NOT SUPPORTED"
  const applyUnsupportedStyle = () => {
    try {
      const label = (arButton.textContent || '').toUpperCase();
      if (label.includes('NOT SUPPORTED')) {
        arButton.classList.add('ar-unsupported');
        arWrap.classList.add('ar-unsupported-wrap');
      } else {
        arButton.classList.remove('ar-unsupported');
        arWrap.classList.remove('ar-unsupported-wrap');
      }
    } catch(_) {}
  };
  applyUnsupportedStyle();
  setTimeout(applyUnsupportedStyle, 50);
  setTimeout(applyUnsupportedStyle, 200);
  setTimeout(applyUnsupportedStyle, 500);
